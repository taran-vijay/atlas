...
Atlas can transcribe speech locally with **START VOICE** in the desktop app.
Click once to begin recording and once more to finish; Atlas sends a valid
transcribed request directly. Typing remains available when you prefer it. Install the local engine
...
Say “Hey Atlas” during a recording session to play a short local listening
chime. If you say only the wake phrase, Atlas stays in recording mode for your
next command. For requests that take longer than a moment, Atlas says “One
moment, sir” using your selected voice. These are fixed local cues—not
generated by the AI—and no audio is sent away. Atlas also repairs a small set
of unambiguous speech-recognition slips before responding.
...
the loaded model between requests. Qwen3 thinking is disabled by default for
quick conversational responses; set `ATLAS_LLM_THINK=true` only when you
prefer slower, deeper reasoning. Raw model speed still depends primarily on
